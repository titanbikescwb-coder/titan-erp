...
export const fiscalService = {
  ...
  async getNotes(): Promise<FiscalNote[]> {
    ...
    const snap = await getDocs(q);
    return snap.docs
      .map(d => ({ id: d.id, ...d.data() } as FiscalNote))
      .sort((a, b) => {
        const tA = (a.dataEmissao as any)?.seconds || (a.dataEmissao as any)?.getTime?.() / 1000 || 0;
        const tB = (b.dataEmissao as any)?.seconds || (b.dataEmissao as any)?.getTime?.() / 1000 || 0;
        return tB - tA;
      });
  },
