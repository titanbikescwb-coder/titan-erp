...
export default function Bicicletas() {
  ...
  toast.error('Erro ao salvar bicicleta.');
} finally {
  ...
  const filteredBikes = bikes.filter(v => {
    const matchesSearch = v.serialNumber?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      v.model.toLowerCase().includes(searchTerm.toLowerCase()) ||
      v.customerName.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesFilter = !filterType || v.type === filterType;
    return matchesSearch && matchesFilter;
  });

  const handleFilterToggle = (type: string | null) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <CardPadrao
          title="MTB / Trail"
          value={bikes.filter(b => b.type === 'MTB').length}
          icon={Layers}
          iconColor="text-emerald-500"
          iconBgColor="bg-emerald-500/10"
          onClick={() => handleFilterToggle('MTB')}
          active={filterType === 'MTB'}
        />
        <CardPadrao
          title="Premium / Carbono"
          value={Math.floor(bikes.length * 0.15)} // Mock indicator for now or just descriptive
          icon={Trophy}
          iconColor="text-amber-500"
          iconBgColor="bg-amber-500/10"
          onClick={() => toast('Módulo de análise premium em desenvolvimento.')}
        />
        <CardPadrao
          title="E-Bikes"
          value={bikes.filter(b => b.type === 'E-Bike').length}
          icon={Zap}
          iconColor="text-blue-500"
          iconBgColor="bg-blue-500/10"
          onClick={() => handleFilterToggle('E-Bike')}
          active={filterType === 'E-Bike'}
        />
      </div>
