...
import { Sale, ServiceOrder, Customer, Budget } from '../domain/types';
import { 
  ShoppingBag, 
  Wrench, 
  TrendingUp, 
  ...
  const filteredHistory = [
    ...
    if (searchTerm) {
      ...
      if (item.historyType === 'sale') {
        matchesId = item.saleNumber?.toString().includes(searchTerm);
      } else if (item.historyType === 'os') {
        matchesId = item.osNumber?.toString().includes(searchTerm);
        matchesDetails = item.bikeDetails?.toLowerCase().includes(searchLower) || false;
      } else if (item.historyType === 'budget') {
        matchesId = item.budgetNumber?.toString().includes(searchTerm);
      }
  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-black/40 p-6 rounded-3xl border border-titan-border flex flex-col justify-between group hover:border-emerald-500/30 transition-colors">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-[10px] font-black text-titan-text-secondary uppercase tracking-widest">Total Gasto</span>
          </div>
          <p className="text-2xl font-black text-white tracking-tighter">{formatCurrency(totalSpent)}</p>
        </div>

        <div className="bg-black/40 p-6 rounded-3xl border border-titan-border flex flex-col justify-between group hover:border-blue-500/30 transition-colors">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-[10px] font-black text-titan-text-secondary uppercase tracking-widest">Transações</span>
          </div>
          <p className="text-2xl font-black text-white tracking-tighter">{purchaseCount} Atendimentos</p>
        </div>

        <div className="bg-black/40 p-6 rounded-3xl border border-titan-border flex flex-col justify-between group hover:border-purple-500/30 transition-colors">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-[10px] font-black text-titan-text-secondary uppercase tracking-widest">Ticket Médio</span>
          </div>
          <p className="text-2xl font-black text-white tracking-tighter">{formatCurrency(averageTicket)}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex bg-white/5 p-1 rounded-2xl w-full sm:w-auto overflow-x-auto no-scrollbar">
            {(['all', 'sales', 'os', 'budgets'] as const).map(tab => (
              <Button 
                ...
                className={cn(
                  ...
                  activeTab === tab ? "bg-titan-primary text-white shadow-xl shadow-titan-primary/20" : "text-white/40 hover:text-white"
                )}
              >
      ...
      <div className="flex-1 space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="flex flex-col items-center justify-center h-60 bg-black/20 rounded-3xl border border-dashed border-titan-border">
            <ClipboardList className="w-12 h-12 text-white/5 mb-2" />
            <p className="text-[10px] font-black text-titan-text-secondary uppercase tracking-widest">Nenhuma transação encontrada</p>
          </div>
        ) : (
          filteredHistory.map((item: any) => {
            ...
            return (
              <div 
                key={item.id} 
                className={cn(
                  "group bg-black/40 rounded-3xl border transition-all cursor-pointer overflow-hidden",
                  isExpanded ? "border-titan-primary ring-1 ring-titan-primary/10 shadow-xl" : "border-titan-border hover:border-titan-primary/30"
                )}
              ...
              >
                <div className="p-5 flex items-center gap-4">
                  <div className={cn(
                    ...
                    type === 'sale' ? "bg-blue-500/10 text-blue-500" : 
                    type === 'os' ? "bg-orange-500/10 text-orange-500" :
                    "bg-white/5 text-titan-text-secondary"
                  )}>
                ...
                <AnimatePresence>
                  {isExpanded && (
                    <motion.div
                      ...
                      className="border-t border-titan-border bg-black/60 overflow-hidden"
                    >
                      <div className="p-6 space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-3">
                            <h5 className="text-[10px] font-black text-titan-text-secondary uppercase tracking-[0.2em] mb-4">Itens / Descrição</h5>
                            <div className="space-y-2">
                              {item.items?.map((prod: any, idx: number) => (
                                <div key={idx} className="flex justify-between items-center text-sm">
                                  <div className="flex items-center gap-2">
                                    <span className="w-5 h-5 rounded bg-white/5 flex items-center justify-center text-[10px] font-bold text-titan-text-secondary">
                                      {prod.quantity}x
                                    </span>
                                    <span className="font-bold text-white uppercase text-[11px]">{prod.name}</span>
                                  </div>
                                  <span className="font-black text-white">{formatCurrency(prod.totalPrice)}</span>
                                </div>
                              ))}
                              {type === 'os' && item.problemDescription && (
                                <div className="mt-4 p-4 bg-black/40 rounded-2xl border border-titan-border">
                                  <p className="text-[10px] font-black text-titan-text-secondary uppercase mb-2">Relato do Problema</p>
                                  <p className="text-sm text-titan-text-secondary/70 font-medium italic">"{item.problemDescription}"</p>
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="space-y-3">
                            <h5 className="text-[10px] font-black text-titan-text-secondary uppercase tracking-[0.2em] mb-4">Informações Detalhadas</h5>
                            <div className="bg-black/40 rounded-2xl p-4 border border-titan-border space-y-3">
                              <div className="flex justify-between items-center">
                                <span className="text-[10px] font-black text-titan-text-secondary uppercase">Atendente</span>
                                <span className="text-[11px] font-bold text-white uppercase">{item.userName || '—'}</span>
                              </div>
                              {item.paymentMethod && (
                                <div className="flex justify-between items-center">
                                  <span className="text-[10px] font-black text-titan-text-secondary uppercase">Pagamento</span>
                                  <span className="text-[11px] font-bold text-white uppercase">{item.paymentMethod.replace('_', ' ')}</span>
                                </div>
                              )}
                              {item.validUntil && (
                                <div className="flex justify-between items-center">
                                  <span className="text-[10px] font-black text-titan-text-secondary uppercase">Validade</span>
                                  <span className="text-[11px] font-bold text-white uppercase">
                                    Até {item.validUntil?.toDate ? format(item.validUntil.toDate(), "dd/MM/yyyy") : '—'}
                                  </span>
                                </div>
                              )}
                              <div className="pt-2 border-t border-titan-border flex justify-between items-center">
                                <span className="text-[10px] font-black text-titan-primary uppercase">Total Bruto</span>
                                <span className="text-base font-black text-white">{formatCurrency(item.totalAmount)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
